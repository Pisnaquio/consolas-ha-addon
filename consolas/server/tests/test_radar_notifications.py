...
class TargetPriceAlertTests(unittest.TestCase):
    ...
    def item(self, *, precio: float, target: float | None, **extra: object) -> dict:
        valuation: dict = {}
        if target is not None:
            valuation["target"] = {
                ...
                "meets": precio <= target,
                "itemPrice": precio,
                "gap": round(precio - target, 2),
            }
        valuation.update(extra.pop("valuation", {}))
        return {"priceAmount": precio, "valuation": valuation, **extra}

    def test_crossing_the_target_interrupts(self) -> None:
        self.assertTrue(is_exceptional(self.item(precio=78.0, target=80.0)))

    def test_being_above_the_target_does_not_interrupt(self) -> None:
        self.assertFalse(is_exceptional(self.item(precio=95.0, target=80.0)))

    def test_without_a_target_nothing_about_targets_interrupts(self) -> None:
        self.assertFalse(is_exceptional({"priceAmount": 40.0, "valuation": {}}))
    ...
    def test_a_small_drop_that_crosses_the_target_notifies_again(self) -> None:
        ...
        why = deserves_notification(self.item(precio=79.0, target=80.0), {"price": 82.0})
        self.assertIn("objetivo", why)

    def test_a_small_drop_that_stays_above_the_target_stays_quiet(self) -> None:
        why = deserves_notification(self.item(precio=95.0, target=80.0), {"price": 98.0})
    ...
    def test_something_already_below_target_does_not_re_notify_forever(self) -> None:
        ...
        why = deserves_notification(self.item(precio=78.0, target=80.0), {"price": 79.0})
        self.assertEqual(why, "")
