...
def email_delivery_config_for_run(
    config: dict[str, str],
    manual_request_id: str = "",
) -> dict[str, str]:
    """Make an explicit UI run always report its canonical result by email."""

    delivery_config = dict(config)
    if manual_request_id.strip():
        delivery_config["AUCTION_WATCH_EMAIL_MODE"] = "always"
...
def prepare_canonical_email_delivery(
    ...
    config = email_delivery_config_for_run(
        config,
        str(manifest.get("manualRequestId") or ""),
    )
...
def attempt_delivery_for_run(
    ...
    *,
    ...
    delivery_config = email_delivery_config_for_run(config, manual_request_id)
...
def run_scan(args: argparse.Namespace) -> int:
    ...
    delivery_config = email_delivery_config_for_run(
        notification_config,
        str(args.manual_request_id or ""),
    )
